...
read trash
read numIt
echo "Introduce the speed-up of the animation (must be a whole number bigger or equal to 1)"
read trash
read speed
...
:'
./EXE_GeneratorChiCalculator "$pathFileWF" "$functionEigenstates" $jmax $nx1 $nx2 $x1min $x1max $x2min $x2max $numIt
...
if [[ $gif == *"G"* ]]; then
    ...
    gnuplot -e "set terminal gif size 1800,900 animate delay 1; set output './OUTPUT_IMAGES/$expLabel $currentDateTime jmax($jmax).gif'; t=0; tmax=$numIt; while(t<tmax){ set multiplot; set origin 0.0, 0.0; set size 0.5, 0.5; clear; set key title; set xrange [$x1min:$x1max]; set yrange [0:0.3]; set xlabel 'Position x'; set ylabel '|Chi^j(x)|'; plot for [i=2:($jmax/2+1)] 'DATA_chiInfo.txt' index t using 1:i title sprintf('|Chi^{%d}(x)|',i-2) w l;
    ...
    set origin 0.5, 0.5; set size 0.5, 0.5; clear; set key title '|WF(x,y)|^2'; set xrange [$x1min:$x1max]; set yrange [$x2min:$x2max]; set xlabel 'Position q1'; set ylabel 'Position q2'; unset hidden3d; set pm3d at bs; set palette rgbformulae 33,13,10; set colorbox; unset surface; splot 'DATA_plotWFInfo.txt' index t title columnheader(1); unset key; unset colorbox; t=t+$speed; unset multiplot;}"
