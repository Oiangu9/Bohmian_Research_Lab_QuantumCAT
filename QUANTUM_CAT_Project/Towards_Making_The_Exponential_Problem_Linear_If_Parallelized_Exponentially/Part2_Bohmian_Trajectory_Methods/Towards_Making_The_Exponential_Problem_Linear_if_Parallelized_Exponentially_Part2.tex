...
One possible way to generalize the functional in equation \eqref{functional1D} is noting that in $N=1$, $\pdv{x(\xi,t)}{\xi}$ is the variant vector of the $\xi$ axis. We could build a generalizing functional for arbitrary $N$ by defining a set of monitor functions $M_1(\vec{\xi,t}),...,M_N(\vec{\xi},t)$ that we wish to equalize blah blah.
takata

en general Euler Lagrange eqts honek die pa N

Pero claro, esto no es exactamente la idea que teniamos originalmente! Pero lo siguiente si que es.

\subsubsection*{\bf (\textgamma.4.2) The N-volume element functional}
Following the original idea, we could try to make the product of a monitor function $M(\vec{\xi},t)$ with the separation increase factor between the trajectories of the mesh elements be as minimum as possible. Since we saw that the $N$-volume local increase factor of the mesh was given by the Jacobian (the determinant of the Jacobian matrix) of the transformation that drives the mesh elements $\vec{x}(\vec{\xi},t)$, we are trying to find the transformation $\vec{x}(\vec{\xi},t)$ that minimizes the functional:
\begin{equation}
...
En $N$ ba izengo da derivada del JAcobianogaz ikusi papelan.

Oin azaldu N dimensiñotara juteko errezena alrebes de forma continum alzuz definidu elemento metriko posibliek, length o voolume or orthogonality por ejemplo del grid. Azaldu funtzionalak eta EL ekuaziñoak emon. Gatxak badie, tipo la del JAcobiano, ba emon 2D kasue eta 3D kasue. Finally esan discrete functionalak al dozuzela erabili, botrh los obtenidos como approx del continuo y los inventados. Ze gauza on azken approachek? BA alzule erabili cosas tipo gradient descent etc eta boundary conditionakl jartzie errezaue dala. Orthogonalen pbmie da boundary conditionak jartziek batzutan ezteuela funtzionetan!
...
% c) Monitor functions erabili tal tal density hau ta bestie. c.2) Ein b+c erdixen izetie monitor function bategaz.
