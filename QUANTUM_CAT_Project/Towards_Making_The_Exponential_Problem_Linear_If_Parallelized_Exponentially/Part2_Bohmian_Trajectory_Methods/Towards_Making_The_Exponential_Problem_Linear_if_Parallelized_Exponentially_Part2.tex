...
- Akabe idazten paralelizaziñoana ta bueno estafilla batelez dala azkenien ze al diskretizar las cosas acabas tratandolas como fluidos tecnicamente hablando digamos.
- Idatzi HJE eta S eboluziñoa ta ikusi ze relaziño dekien con G,J potentials exaktamente
- Idatzi Norsenena de cómo esconder las cosas-> Dynamical eqts for G,J moduen azaldute
- Adapte dyanmic derivatives
- Adapte label space derivatives, hau igual ni falta que hace
- Adapte custom trajectories/grids
- Sartun marrazkixek ta euren kaptxionak
...
EN CONDIITONAL WAVEUFNCTIONS ES CASI LA PARTICULA AL QUE GUIA A LAS FUNCIONES DE ONDA CONDICIONADAS, A SUS PILOT WAVES...

Lo que queda pensjant en la partially lagrnagian de hje y ce es precisamente el G y la J! Lol.
\mybox{
\subsection*{Swinging From exponential time to linear using Lagrangian degrees and (exponential) parallelization}
In this box we will show how changing the Eulerian to a Lagrangian frame leaves us turn the temporally exponential problem of the quantum many body into a linear one if we parallelize exponentially in parallel processes for which we allow messaging (with reasonable overhead). We will see that the partially Lagrangian frame allows us decide whether we put more weight in the temporal complexity of each process (setting more of the degrees in the Eulerian frame), or use more processes in exchange of simpler computations for them (relatively more Lagrangian degrees). In the limit of the Fully Lagrangian frame, we will have maximal parallelizability (which we have already commented), and in the limit of the Fully Eulerian frame we will have a more monolithic computation thread.\\

In what follows we will analyze qualitatively the two limit cases where there are still both Lagrangian and Eulerian degrees: having a single Lagrangian one and having them all but one.\\

In all the equations so far we have seen that it is roughly mandatory that we evolve an ensemble of complementary conditional wavefunctions (or actions and densities) with their respective trajectories (unless we adopt another strategy like evolving the derivatives). This was because all the dynamical equation for each complementary function (wavefunction, action or density) required the knowledge of the derivatives in the directions we evaluate in the trajectory. Having complementary functions for them give us an idea of how the function works in that direction, but only for the Lagrangian axes traversing the Eulerian position where the trajectory coupling the complementary set of functions is. Thus we required to evolve several trajectories, such that we end up having an image of the full functions and thus be able to know its variation in any direction. That said, we see that there is a freedom of choosing complementary conditional functions with more or less Lagrangian degrees. What will the price be?\\

{\bf Only one Lagrangian degree\\ \\}
If we left only one of the degrees of freedom as Lagrangian, say $x_N$, we would have that a conditional function $f(x_1,...,x_{N-1},x_N^\xi(t))$ is a slice of the full function whose support is a hyperplane of $\R^N$.\footnote{In all the explanation, it is convenient to imagine $N=3$. As such, if we consider as Lagrangian degree the $x_3$, the support of the conditional functions of shape $f(x_1,x_2,x_3^\xi(t),t)$ will be a moving plane in $x_1,x_2$, sliced at $x_3=x_3^\xi(t)$).} To know the variation $\pdv{}{x_N}f(x_1,...,x_{N-1},x_N)$ for $x_N=x_N^\xi(t)$ in each point $(x_1,...,x_{N-1})$ we consider, we will have to pay attention to the variation of the function along $x_N$ per each $(x_1,...,x_{N-1})$. This means that, as we will have a collection of trajectories $\{\vec{\xi}^j\}_{j=1}^M$ each of which has its own conditional-hyperplane function $f(x_1,...,x_{N-1},x_N^\xi(t))$ centered at $\{x_N=x_N(\vec{\xi}^j,t)\}_j$, we could consult the different contiguous hyperplanes and numerically compute the derivative. See Figure \ref{fig:hyperplane}.\\

The approach would then follow to first compute the correlation terms (that include the derivatives in the Lagrangian axes\footnote{Which is why they are called correlation terms, they introduce the correlation of the active conditional function with the rest of conditional functions paralllel and transversal to it} of one time step in parallel for each hyperplane labeled by each underlying fluid element $\vec{xi}^j$ and each considered point $x_1,...,x_N$ in each hyperplane. Then in parallel execute one time step evolution for the conditional functions as given in the equations above, employing the necessary algorithm for an $m$ dimensional Eulerian function (in the case of $G$ and $J$ for instance, we could emlpoy a Crnack Nicholson algorithm). Then once all the hyperplane-conditional functions are updated, compute again the correlation potentials in parallel, and repeat the loop.}

\mybox{
As a little computational detail, if we choose the Lagrangian degrees to move (say, according to the Bohmian velocity field), an initially regular grid in the $x_N$ axis (of different hyperplanes) will get unstructured. The distances between the hyperplanes will become irrelgular. This will make the following derivatives in the Lagrangian axis be more difficult to compute. We could then use fitting techniques among others, or instead, we could change the derivatives to derivatives in the label space as explained in Section (III.e). Alternatively, we could chose to fix the trajectory $x_N^\xi(t)=\xi\ \forall t>0$ (since remember the equations do not force us to make them be Bohmian trajectories). In such a case, the derivatives would always be in a regular grid and could trivially be computed  numerically. This would be a subtle way to turn back to the Eulerian fixed grid though.\\

To have an example of the complexity reduction, we will assume to use the $G$ and $J$ correlation potential method \eqref{SE.GJ}. As explained in \cite{NireTFGie}, evolving a time step of the full $N$ dimensional Schrödinger Equation \eqref{SE} has an exponential complexity $O(M^N)$ with $M$ the average number of points considered in the discretization of each axis. A time step of the $N-1$ dimensional Schrödinger Equation that would be left for the Eulerian degrees after computing the correlation potentials in the $G$, $J$ method has a complexity $O(M^{N-1})$. Computing the numerical derivatives on the axis of the Lagrangian degree has a complexity of about $O(M)$ operations (we consider there are $M$ fluid elements distributed in $x_N$) per each of the $O(M^{N-1})$ points in a hyperplane. These derivatives are independent between them, so if we compute the $O(M)$ derivatives derivatives of each hyperplane point in parallel, using $O(M^{N-1})$ threads, we can compute the correlation potentials for the next iteration. Then each hyperplane-conditional wavefunction is evolved in parallel using $O(M)$ threads taking each again $O(M^{N-1})$ time. Overall, using $O(M^{N-1})$ parallel threads, the exact evolution of the Schrödinger Equation can be obtained in $O(M+M^{N-1})=O(M^{N-1})$ time, one $M$ factor less than using all the degrees as Eulerian.}
...
{\bf All Lagrangian but one\\}

{\bf Using all degrees as Lagrangian\\}

{\bf Was it really impossible to achieve this with the fully Eulerian frame? No.\\}
Si trajs still kalro, teknikamente es lo mismo que si fuese Euleriano, pero puedes hacer el mismo algorithm. asike bueno, esto es solo una forma posible de pensar las cosas.
