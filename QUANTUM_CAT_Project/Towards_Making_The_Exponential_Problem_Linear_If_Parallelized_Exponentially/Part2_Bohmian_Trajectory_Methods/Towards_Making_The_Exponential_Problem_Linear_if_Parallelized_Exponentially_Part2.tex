...
In this section, we will explore an intermediate approach between the Lagrangian and Eulerian frames. For this, we will consider that part of the system is observed in a Lagrangian frame, while the other part in an Eulerian frame. The degrees of freedom described on the Eulerian frame will be denoted by $\vec{x}=(x_1,...,x_m)\in\R^m$ while the rest of degrees of freedom $\vec{y}=(x_{m+1},...,x_N)\in\R^{N-m}$ will be described as seen by Lagrangian frame fluid elements. If we need to refer to both kinds of variables at once, we will use $\vec{\x}=(\vec{x}, \vec{y})=(x_1,...,x_N)\in \R^N$.

Once again, we will label each fluid element using real vectors $\vec{\xi}\in\Omega_0\subseteq \R^N$ representing their position $\x$ in configuration-space at a reference time $t_0$, thus $\vec{\xi}:=\vec{\x}( \vec{\xi},t_0)$. In particular, we will use $\vec{\xi}_x=(\xi_1,...,\xi_m)$ to denote the Lagrangian labels of the degrees of freedom treated in the Eulerian frame $\vec{x}$, while $\vec{\xi}_y=(\xi_{m+1},...,\xi_N)$ will denote the labels of the Lagrangian degrees of freedom. In general $\vec{\xi}=(\vec{\xi}_x,\vec{\xi}_y)\in\R^N$.

Since we assume each fluid element follows the trajectory given by a continuous velocity field in configuration-space, by the Picard-Lindelöf theorem, there must exist a diffeomorphism $\vec{\x}(\vec{\xi},t)\equiv \vec{\x}^\xi(t) \quad \xi \in \Omega \subset \R^N$ giving their trajectories at all times. Since we will only treat explicitly in the Lagrangian frame the degrees of freedom in $\vec{y}$, it is convenient to denote by $\vec{y}^\xi(t)=(x_{m+1}^\xi(t),...,x_N^\xi(t))=(x_{m+1}(\xi,t),...,x_N(\xi,t))$, the set of trajectories for the subsystem $\vec{y}$.

In order to have a geometric understanding of what having only part of the system in a Lagrangian frame means, consider a field $f(\vec{\x},t)$, where we evaluate part of it along the trajectory $\vec{y}^\xi(t)$. We would then be considering the so called {\bf conditional function} $f(\vec{x}, \vec{y}^\xi(t),t)=f(\vec{x}, \vec{\xi},t)\equiv f^\xi(\vec{x},t)$. Depending on the trajectory $\vec{\xi}$ we choose, the function $f(\vec{x}, \vec{y}^\xi(t),t)$ will represent a different moving   $m$ dimensional "slice" of the $N$ dimensional field $f(\vec{\x},t)$. It is this why we will also call each possible set of degrees of freedom $\vec{y}$ a "transversal section" of the system. You can see in Figure \ref{fig:slices} a visual representation of some conditional fields along different trajectories.
...
Note that if we describe the conditional function $f(\vec{x}, \vec{y}^\xi(t),t)$, we can naturally also considered a coupled conditional function $f(\vec{x}^\xi(t), \vec{y},t)$, where we have the trajectory of the fluid element $\vec{\xi}$ is $\vec{\x}(\vec{\xi},t)=(\vec{x}^\xi(t),\vec{y}^\xi(t))$. Or in fact

Baia klaro, y si el fluid element solo lo defino en el subespacio de y? Porque necisto yo el full space?! De hecho ni falta que hace! Si procuras que los fluid elemnts en y no se crucen (ke no es lo mismo ke en el full space), ya va bien para con los slices, te definiran toda la funcion al final!
