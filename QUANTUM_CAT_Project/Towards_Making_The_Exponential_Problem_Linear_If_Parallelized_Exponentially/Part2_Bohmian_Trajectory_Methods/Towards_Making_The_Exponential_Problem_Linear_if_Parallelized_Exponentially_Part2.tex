...
\begin{figure}[h!]
  \centering
    \includegraphics[width=0.75\linewidth]{6slices_1d.png}
  \caption{Depiction of conditional functions in $N=2$, where the $y$ axis is considered in the Lagrangian frame and $x$ in the Eulerian one. In the center left, there are two axes representing the initial distribution of points $\xi_y$ in $y$ in an equidistant fashion, and their (non-equidistant) positions $y(\xi_y,t)$ after a time $t$. Each fluid element in $y$ has its own associated conditional function $f(x,y(\xi_y,t),t)$. If we see the conditional functions from the full function's $f(x,y,t)$ perspective, we can acknowledge they are slices, as depicted in the right (above the initial function and below the function at time $t$). These slices can give us an intuition of the full function.  }
  \label{fig:slices}
\end{figure}
...
\begin{figure}[h!]
  ...
  \caption{In blue we can see the affine supports of several pairs of CWF-s (slices of the WF) built by following the complementary function idea. In each intersection we have enough information as for evolving a Bohmian trajectory. }
  \label{fig:over}
\end{figure}
